...
def framesig(sig, frame_len, frame_step, winfunc=lambda x: numpy.ones((1, x))):
    ...
    if signal_len <= frame_len:
        ...
        numframes = 1 + int(math.ceil((1.0 * signal_len - frame_len) / frame_step))
    padlen = int((numframes - 1) * frame_step + frame_len)
    zeros = numpy.zeros((padlen - signal_len,))
    padsignal = numpy.concatenate((sig, zeros))  # fill origin signal using zero
